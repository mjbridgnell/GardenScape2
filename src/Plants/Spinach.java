...
public class Spinach {
    private long growTime = Global.getGrowTime("spinach"); // seconds
    private double percentGrown = 0;
    private int squareSize = 100;
    public ImageView tileImage; // Changed to ImageView
    ...
    private Label spinachInfoLabel = new Label("Spinach Age: 0"
            + "\nPercent Grown: 0%"
            + "\n30 days to grow"
            + "\nPlant Feb-Mar or Oct-Nov"
            + "\nWater several times a week, 1-1.5in"
            + "\n9 Seeds/Plants per square foot");

    private void addHighlightEffect(StackPane tileContainer, Rectangle highlight, Scene gameScene, Spinach spinach) {
        spinach.setInfoLabelVisible(false);

        Label harvestPopup = new Label("9 Spinach Plants Harvested!");
        ...
        if (!rootChildren.contains(spinach.spinachInfoLabel)) {
            rootChildren.add(spinach.spinachInfoLabel);
        }
        ...
        updatePopupPositions(gameScene, spinach, harvestPopup);

        // Add a listener to adjust the popups when the window is resized
        gameScene.widthProperty()
                .addListener((obs, oldVal, newVal) -> updatePopupPositions(gameScene, spinach, harvestPopup));
        ...
        tileContainer.setOnMouseEntered(event -> {
            ...
            spinach.setInfoLabelVisible(true); // Show info box when hovering
            spinach.spinachInfoLabel.toFront(); // Bring the popup to the front
        });

        tileContainer.setOnMouseExited(event -> {
            highlight.setFill(Color.TRANSPARENT); // Set back to transparent
            spinach.setInfoLabelVisible(false); // Hide the info box
        });

        tileContainer.setOnMouseClicked(event -> {
            if (spinach.harvestable) {
                Global.spinachHarvested = Global.spinachHarvested + 9;
                Global.incrementPlants();
                spinach.setImage("Plants/PlantSprites/immatureSpinach.png");
                spinach.harvestable = false;
                spinach.timestamp = System.currentTimeMillis();

                Global.updatePlanttoCell(spinach.col, spinach.row, timestamp);

                Seeds.incrementSeeds("spinach");
    ...
    private void updatePopupPositions(Scene gameScene, Spinach spinach, Label harvestPopup) {
        spinach.spinachInfoLabel.setTranslateX(gameScene.getWidth() - gameScene.getWidth() / 1.5);
        spinach.spinachInfoLabel.setTranslateY(gameScene.getHeight() - gameScene.getHeight() * 1.2);
    ...
    public void loadSpinach(int col, int row, GridPane gridPane, Scene gameScene) {
        this.col = col;
        this.row = row;
    ...
    public void placeTile(GridPane gridPane, Scene gameScene) {
        Global.setCurrentPlant("spinach");

        // Check if month is correct
        if (Global.month != 2 && Global.month != 3 && Global.month != 10 && Global.month != 11) {
            Global.setCurrentPlant(null);
            Label seasonPopUp = new Label("Cannot plant spinach in " + Global.season + "!"
                    + "\nPlant Feb-Mar or Oct-Nov.");
        ...
        gridPane.setOnMouseClicked(mouseEvent -> {
            if (Global.getCurrentPlant() != "spinach")
                return;

            col = (int) (mouseEvent.getX() / squareSize);
            row = (int) (mouseEvent.getY() / squareSize);

            if (Global.getPlantAtCell(col, row) == "spinach") {
                return;
            } else {
            ...
            // Save spinach
            Plant spinachPlant = new Plant("spinach", timestamp);
            Global.addPlanttoCell2(col, row, spinachPlant);

            Seeds.incrementSeeds("spinach");

            StackPane tileContainer = new StackPane();
            tileContainer.getChildren().addAll(spinach.tileImage, spinach.highlight);
            addHighlightEffect(tileContainer, spinach.highlight, gameScene, spinach);
            gridPane.add(tileContainer, col, row);
            Global.incrementPlants();
        });
    ...
    public Spinach(int squareSize) {
        this.squareSize = squareSize;

        spinachInfoLabel.setStyle("-fx-background-color: lightgray; -fx-padding: 5px; -fx-border-color: black;");

        tileImage = new ImageView(new Image("Plants/PlantSprites/immatureSpinach.png"));
        tileImage.setFitWidth(squareSize);
        tileImage.setFitHeight(squareSize);
    ...
    public Spinach(int squareSize, long timestamp) {
        this.squareSize = squareSize;
    ...
    private void updateLabel() {
        ...
        spinachInfoLabel.setText("Spinach Age " + days + ":" + hours + ":" + minutes + ":" + seconds
                + "\nPercent Grown: " + formattedPercentGrown + "%"
                + "\nSpinach Harvested: " + Global.spinachHarvested
                + "\n30 days to grow"
                + "\nPlant Feb-Mar or Oct-Nov"
                + "\nWater several times a week, 1-1.5in"
                + "\n9 Seeds/Plants per square foot");
        ...
        if (secondsElapsed > 10) {
            setImage("Plants/PlantSprites/matureSpinach.png");
            harvestable = true;
        }
    }

    public void setInfoLabelVisible(boolean visible) {
        spinachInfoLabel.setVisible(visible);
    }
